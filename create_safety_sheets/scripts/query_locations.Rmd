...
ne_areas <- services_fn(location_type = locations, dist = 400000, # in meters ~250 miles 
...
locs <- vector(mode = "list", length = length(location_type))
for (i in 1:length(location_type)){
  
  locs[[i]] <- google_places(place_type = location_type[i], location=c(39.26, -114.8),
                rankby = 'distance', key = SoS_gkey)
}
names(locs) <- location_type
searches <- dplyr::bind_rows(locs)


# now ensure areas of interest are within ~50 miles of field office boundaries

ggplot() +
  geom_sf(data = ne_areas, aes(color = Service))
