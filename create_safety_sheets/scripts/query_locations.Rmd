...
ne_areas <- lapply(f_areas_se, distAZE, places_data = nvp) %>% bind_rows()
se_areas <- lapply(f_areas_sw, distAZE, places_data = nvp) %>% bind_rows()
...
st_write(ne_areas, file.path(p, 'NE', 'NE.shp'))
st_write(nw_areas, file.path(p, 'NW', 'NW.shp'))
st_write(se_areas, file.path(p, 'SE', 'SE.shp'))
st_write(sw_areas, file.path(p, 'SW', 'SW.shp'))
