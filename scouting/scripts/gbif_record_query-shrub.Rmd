...
target_taxa <- read.csv(file.path(p, 'target_shrubs-2023.csv')) %>% 
  pull(Species)
...
<<<<<<< HEAD
file.path(p, 'GBIF_query-SHRUB' )

spp_occ <- read.delim(
  file.path(file.path(p, 'GBIF_query-SHRUB' ), 
=======
file.path(p, 'GBIF_query1' )

spp_occ <- read.delim(
  file.path(file.path(p, 'GBIF_query1' ), 
>>>>>>> fa27785904615721951e0d3d27906b1fef6b69b2
...
st_write(spp_occ, file.path('../data', 'GBIF', 'GBIF_cleaned-SHRUBS.shp'), append = F)
...
st_write(bien_stuff, file.path('../data', 'BIEN', 'BIEN_cleaned-SHRUBS.shp'), append = F)
...
bien <- st_read(file.path('../data', 'BIEN', 'BIEN_cleaned-SHRUBS.shp'), quiet = T)
gbif <- st_read(file.path('../data', 'GBIF', 'GBIF_cleaned-SHRUBS.shp'), quiet = T)
...
st_write(consensus, file.path('../data', 'BIEN-GBIF', 'BIEN-GBIF_cleaned-SHRUBS.shp'), append = F)
...
target_taxa_lkp <- read.csv(file.path(p, 'target_shrubs-2023.csv')) %>% 
...
gbif_bien <- st_read(file.path('../data', 'BIEN-GBIF', 'BIEN-GBIF_cleaned-SHRUBS.shp'))
...
st_write(hist_data, file.path('../data', 'SDM-occ', 'SDM-occ-SHRUBS.shp'), append = F)
